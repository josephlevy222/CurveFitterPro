...
//
/// Store arrays as raw IEEE-754 bytes. This works for any Double value including ±infinity and NaN, avoids all JSON encoding issues, and is fast (just a memcpy).
...
// Nil-safe Double: store as two Doubles [isPresent, value] where isPresent is 1.0 if value exists, 0.0 if nil
...
// MARK: - Decode cache
//
/// Keyed on project id so projects don't collide. The dictionary ensures  multiple active documents don't overwrite each other's caches.

private var _dpCache:    [UUID: (key: Data, value: [DataPoint])]    = [:]
private var _paramCache: [UUID: (key: Data, value: [FitParameter])] = [:]

// MARK: - Project
//
/// All stored properties are types CoreData/SwiftData can materialise: String, Int, Bool, Double, Date, Data.
...
extension Project {
	...
	var dataPoints: [DataPoint] { //Binding<[DataPoint]> { Binding(
		get {
			// Use cached decode if the underlying blob hasn't changed
			if let c = _dpCache[id], c.key == dpXData { return c.value }
		...
		set {
			let newXData = doublesToData(newValue.map(\.x))
			dpXData         = newXData
			...
			// 🛑 Update the cache instead of nilling it.
			// This preserves the existing UUIDs so SwiftUI doesn't destroy your rows.
			_dpCache[id]    = (newXData, newValue)
			modifiedAt      = Date()
		}
	}
	...
	var parameters: [FitParameter] {
		get {
			...
			if let c = _paramCache[id], c.key == paramInitData { return c.value }
		...
		set {
			...
			let newInitData = doublesToData(newValue.map(\.initialValue))
			paramInitData   = newInitData
			...
			// 🛑 Update the cache instead of nilling it to preserve UUIDs.
			_paramCache[id] = (newInitData, newValue)
		}
	}
