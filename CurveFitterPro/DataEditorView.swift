...
struct DataPointRow: View {
	...
	// 🚀 Track the active background typing delay timer
	...
	var body: some View {
		...
		// 🚀 DEBOUNCE PIPELINE: Fires after 1.5 seconds of typing inactivity
		.onChange(of: pStr) {
			debounceTask?.cancel()
			debounceTask = Task {
				try? await Task.sleep(for: .seconds(1.5)) // Adjust latency flavor here
				guard !Task.isCancelled else { return }
				commitChanges()
			}
		}
		// Clean up dangling timers if a row is deleted or discarded mid-type
		.onDisappear {
			debounceTask?.cancel()
		}
	...
	private func syncFromModel() {
		guard !isFocused else { return } // 💡 CRITICAL Don't reformat if still typing here
