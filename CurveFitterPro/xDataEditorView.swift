...
struct DataPointRow: View {
	...
	private func cell(text: Binding<String>, col: DataEditorView.Field.Column, width: CGFloat? = nil) -> some View {
		NumericTextField("", numericText: text,
			...
			if !editing {
				// Defer the model update by one run loop. By then UIKit has already
				// transferred first responder to the newly-tapped field, so the
				// re-render triggered by commitChanges() can't disrupt that transfer
				// and the keyboard stays visible throughout.
				Task { @MainActor in commitChanges() }
			}
		},
						 onCommit: {
			// Done button: commit immediately, then advance or dismiss
			commitChanges()
...
struct DataPointRow: View {
	...
	private func cell(text: Binding<String>, col: DataEditorView.Field.Column, width: CGFloat? = nil) -> some View {
		NumericTextField("", numericText: text, onEditingChanged: { editing in
			if !editing { commitChanges() }   // ← fires on any focus loss
