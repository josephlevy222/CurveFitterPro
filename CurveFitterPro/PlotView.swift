...
struct PlotView: View {
	...
	@State private var confidenceSize: CGSize = .zero
	@State private var residualToggleSize: CGSize = .zero
	...
	private var remainingHeight: CGFloat {
		max(0, viewPortSize.height - confidenceSize.height - residualToggleSize.height)
	}
	private var plotHeight: CGFloat? {
		project.showResiduals ? remainingHeight * 0.6 : nil
	}
	private var residualHeight: CGFloat {
					...
					Color.clear
						...
						.onChange(of: geo.size) { _,newSize in
							DispatchQueue.main.async {
								if viewPortSize.height != newSize.height {
	...
	private func buildPlotData() {
		...
		let annotationText: AttributedString? = {
			...
			if !parameterSubstitution.isEmpty {
                ...
                let mathExpr = EquationFormatter.formatToAttributedStringEquation(parameterSubstitution, fontSize: fontSize)//, includeLHS: true)
				finalAttr.append(mathExpr)
			}
//            if !parameterSubstitution.isEmpty {
//                finalAttr.append(AttributedString("\n"))
//                
//                // 1. Separate LHS ("y = ...") if an equals sign is present
//                let expressionString: String
//                let prefix: String
//                
//                if parameterSubstitution.contains("=") {
//                    let parts = parameterSubstitution.split(separator: "=", maxSplits: 1)
//                    prefix = "\(parts[0].trimmingCharacters(in: .whitespaces)) = "
//                    expressionString = parts[1].trimmingCharacters(in: .whitespaces)
//                } else {
//                    prefix = ""
//                    expressionString = parameterSubstitution
//                }
//                
//                // 2. Parse and format through ExpressionParserAndPresenter
//                var mathAttr = AttributedString(prefix)
//                if let compiled = try? CompiledExpression(source: expressionString) {
//                    mathAttr.append(compiled.format())
//                } else {
//                    mathAttr.append(AttributedString(expressionString)) // Graceful fallback
//                }
//                
//                // 3. Apply desired font size
//                mathAttr.font = .system(size: fontSize)
//                
//                finalAttr.append(mathAttr)
//            }
    ...
    private func fittedEquation(result: FitResult) -> String {
        ...
        for p in sorted {
            ...
            let s: String = (abs(v) >= 1000 || (abs(v) < 0.01 && v != 0))
                ? String(format: "%.3g", v)
                : String(format: "%.4g", v)//[cite: 3]
            
            // 1. Determine if this parameter is at the start of the expression string
            let trimmedEq = eq.trimmingCharacters(in: .whitespaces)
            let isLeading = trimmedEq.hasPrefix(p.name) || trimmedEq.hasPrefix("y = \(p.name)")
            
            // 2. Unwrapped negative for leading term; parenthesize all others
            let formatted = (v < 0 && !isLeading) ? "(\(s))" : s
            
            // Replace original parameter name
            eq = eq.replacingOccurrences(of: p.name, with: formatted)//[cite: 3]
            
            // Replace subscript variations if applicable
            if p.name.count > 1 {//[cite: 3]
                let firstLetter = p.name.prefix(1)//[cite: 3]
                let remainingPart = p.name.dropFirst()//[cite: 3]
                
                let plainSubscript = "\(firstLetter)_\(remainingPart)"//[cite: 3]
                let bracketedSubscript = "\(firstLetter)_[\(remainingPart)]"//[cite: 3]
                
                eq = eq.replacingOccurrences(of: plainSubscript, with: formatted)//[cite: 3]
                eq = eq.replacingOccurrences(of: bracketedSubscript, with: formatted)//[cite: 3]
            }
        }
        return eq
    }
...
//				s = String(format: "%.3g", v)
//			} else {
//				s = String(format: "%.4g", v)
//			}
//			// Parenthesize negative values so substitution into any context is unambiguous.
//			// e.g. "2 * param" with param = -3 becomes "2 * (-3)" not "2 * -3".
//			return (v < 0) ? "(\(s))" : s
//		}
//		
//		var eq = project.modelEquation.isEmpty ? project.modelExpression : project.modelEquation
...
//			
//			// 2. Replace potential subscript variations generated by EquationFormatter
//			if p.name.count > 1 {
	...
	private func buildPlotData() {
		...
		let annotationText: AttributedString? = {
			...
			if !parameterSubstitution.isEmpty {
				...
				// 1. Core Evaluation & Simplification (Combines constants, folds x * x -> x^2)
				let optimizedEquation = EquationFormatter.simplify(parameterSubstitution)
				
				// 2. Visual Layout Polish (Applies font weights, sizes, and baseline offsets)
				let mathExpr = EquationFormatter.formatToAttributedString(optimizedEquation, fontSize: fontSize, includeLHS: true)
