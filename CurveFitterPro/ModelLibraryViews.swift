                            ...
                            ForEach(models) { model in
                                Button {
                                    ...
                                    VStack(alignment: .leading, spacing: 4) {
                                        ...
                                        Text(EquationFormatter.formatToAttributedStringEquation(model.expression))
...
struct CustomModelSheet: View {
    ...
    @State private var expressionSelection: TextSelection?
    ...
    var body: some View {
        NavigationStack {
            Form {
                ...
                Section {
                    TextField("e.g. a * exp(-b * x) + c", text: $expression, selection: $expressionSelection)
                        .font(.system(.body, design: .monospaced))
                        .autocorrectionDisabled()
                        .textInputAutocapitalization(.never)
                        .onChange(of: expression) { _, newValue in
                            // "\s" -> "σ", "\S" -> "Σ", etc., typed anywhere in the field.
                            if let (rewritten, newCursor) = applyGreekPhoneticSubstitution(to: newValue, selection: expressionSelection) {
                                expression = rewritten
                                expressionSelection = TextSelection(insertionPoint: newCursor)
                                // Setting `expression` above re-triggers this onChange with the
                                // rewritten text; that pass finds no "\<letter>" pattern left and
                                // falls through to parseExpression() below, so skip it here to
                                // avoid parsing the transient pre-substitution text.
                                return
                            }
                            parseExpression()
                        }
